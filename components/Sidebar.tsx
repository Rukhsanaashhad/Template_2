...
import { categoriesData, headerData } from '@/constants';
    ...
    className="min-w-72 max-w-96
     bg-darkColor text-white/70
      ...
      href={`/category/${item?.title}`}
       ...
       ${
        pathname ===  `/category/${item?.title}`  && "text-white"}`}
      >
        {item?.title}
        </Link>
    ))}
