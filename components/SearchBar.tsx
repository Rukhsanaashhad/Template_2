...
const SearchBar = () => {
  ...
  const fetchProducts = useCallback(async () => {  
    ...
    try {  
        // Define a search variable for the query  
        const query = `*[_type == "product" && name match $search] | order(name asc)`;  
        const params = { search: `*${search}*` };  // Use wildcards for matching  

        const response = await client.fetch(query, params);  
         // Check the response  
        setProducts(response); // Update state with fetched products  
    } catch (error) {  
        console.log('Error fetching products:', error); // Log error details  
    } finally {  
        setLoading(false);  
    }  
}, [search]);
            ...
            <DialogTitle className="mb-1">
              ...
              <form className="relative" onSubmit={(e)=>e.preventDefault()}>
                <Input placeholder="Search Your Product Here..."
                 ...
                 hover:bg-darkColor hover:text-white hoverEffect ${search ? "bg-darkColor text-white": "bg-darkColor/10"}`}>
          ...
          <div className="w-full h-full overflow-y-scroll border border-darkColor/20 rounded-md">
            <div>
                  ...
                  className="bg-white overflow-hidden border-b last:border-b-0">
                   ...
                   <Link
                    href={`/product/${product?.slug?.current}`}
                     className="h-20 w-20 md:h-24 md:w-24 flex-shrink-0
                   border border-darkColor/20 rounded-md overflow-hidden group"
                    onClick={()=>setShowSearch(false)} >
                    {product?.image && (<Image width={200} height={200} src={urlFor(product?.image).url()} alt="productImage"
              ...
              <div className="text-center py-10 font-semibold tracking-wide">
                {search && !loading ? 
                <p>
                  Nothing Match With The Keyboard
                ...
                <p className="text-green-600 flex items-center justify-center gap-1">
                  <Search className="w-5 h-5"/>Search and Explore your Product From Max.
                  </p>}
                </div>}
            </div>
          </div>
