...
const ProductCard = ({product} : {product: Product}) => {  
  ...
  <div className="group text-sm rounded-lg overflow-hidden">
    ...
    height={500}
     alt="product"
      priority 
      className={`w-full h-72 object-contain overflow-hidden  hoverEffect ${product?.quantity !== 0 && "group-hover:scale-105"}`} />
    </Link>   
  )}
      {product?.quantity === 0 && (
        <div className="absolute top-0 left-0 w-full h-full bg-darkColor/50 items-center justify-center" >
